...
import { WS_OPEN_STATE, broadcastToUser } from '@/modules/websocket/index.js';
...
function isWatcherTargetFile(provider: LLMProvider, filePath: string): boolean {
  if (provider === 'claude') {
    // Claude sessions are single .jsonl files per session in a flat directory.
 ...
 */
async function buildSessionUpsertedEvent(updatedProviderSessionId: string): Promise<{ event: string; userId: number | null } | null> {
...
async function flushPendingWatcherUpdate(): Promise<void> {
  ...
  try {
    ...
    // Broadcast per user so sessions never leak across user accounts.
    const eventsByUser = new Map<number | null, string[]>();
    for (const updatedSessionId of queuedUpdate.updatedSessionIds) {
      const result = await buildSessionUpsertedEvent(updatedSessionId);
      if (result) {
        const key = result.userId ?? null;
        let list = eventsByUser.get(key);
        if (!list) {
          list = [];
