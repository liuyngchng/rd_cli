...
export function createProviderRuntimeService(
  ...
  const run = (
    ...
    // Ownership check: only the session owner may run against it.
    // The Chat WebSocket handler already performs this check before dispatching,
    // but REST/SSE callers reach this path directly, so the check lives here as
    // a defense-in-depth measure.
    const sessionId = typeof options.sessionId === 'string' ? options.sessionId : '';
    if (sessionId) {
      const session = sessionsDb.getSessionById(sessionId);
      const sessionUserId = session?.user_id ?? null;
      const writerUserId = writer?.userId != null ? Number(writer.userId) : null;
      if (sessionUserId != null && writerUserId != null && sessionUserId !== writerUserId) {
        const error = new Error(`Session "${sessionId}" belongs to another user.`);
        (error as unknown as Record<string, unknown>).code = 'SESSION_FORBIDDEN';
