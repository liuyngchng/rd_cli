...
export const runMigrations = (db: Database) => {
  try {
    ...
    // Multi-user migration: add user_id column to projects (existing data defaults to NULL)
    {
      const projectsTableInfo = getTableInfo(db, 'projects');
      ...
      addColumnToTableIfNotExists(
        ...
        projectColumnNames,
        'user_id',
        'INTEGER DEFAULT NULL REFERENCES users(id) ON DELETE SET NULL'
      );
    }
    ...
    // Multi-user migration: add user_id column to sessions (existing data defaults to NULL)
    {
      const sessionsTableInfo = getTableInfo(db, 'sessions');
    ...
    // Backfill: the first user (lowest id) is the admin
    const firstUser = db.prepare('SELECT id, role FROM users ORDER BY id ASC LIMIT 1').get() as { id: number; role: string } | undefined;
