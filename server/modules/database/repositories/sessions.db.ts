...
const SESSION_ROW_COLUMNS =
  'session_id, provider, provider_session_id, project_path, user_id, jsonl_path, custom_name, model, isArchived, created_at, updated_at';
...
export const sessionsDb = {
  ...
  createSession(
    ...
    db.prepare(
      `INSERT INTO sessions (session_id, provider, provider_session_id, custom_name, project_path, user_id, jsonl_path, isArchived, created_at, updated_at)
   ...
   */
  createAppSession(sessionId: string, provider: string, projectPath: string, userId?: number): string {
    const db = getConnection();
    const normalizedProjectPath = normalizeProjectPathForProvider(provider, projectPath);
    ...
    ).run(sessionId, provider, normalizedProjectPath, userId ?? null);
   ...
   *
   * Primary intention: an external tool may expose a new session before the
   * websocket runtime reports that same provider id back to our app. At that
   * moment the sidebar already has an optimistic
