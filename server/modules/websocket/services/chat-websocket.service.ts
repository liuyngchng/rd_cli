...
async function handleChatSend(
  ...
  // Ownership check: only the session owner (or a user without a user_id, i.e.
  // legacy PLATFORM mode) may send to a session.  Sessions with a NULL user_id
  // (pre-migration rows) are allowed for backward compat.
  if (session.user_id != null && userId != null && session.user_id !== userId) {
    sendProtocolError(
      ws,
      'SESSION_FORBIDDEN',
...
async function handleChatAbort(
  ...
  const session = sessionsDb.getSessionById(sessionId);
  if (session && session.user_id != null && userId != null && session.user_id !== userId) {
    sendProtocolError(ws, 'SESSION_FORBIDDEN', `Session "${sessionId}" belongs to another user.`, sessionId);
    return;
  }
...
function handleChatSubscribe(
  ...
  for (const target of targets) {
    ...
    // Ownership check: skip sessions that belong to another user.
    const session = sessionsDb.getSessionById(sessionId);
    if (session && session.user_id != null && userId != null && session.user_id !== userId) {
      continue;
    }
