...
export const api = {
  ...
  unifiedSessionMessages: (sessionId, _provider = 'pi', { limit = null, offset = 0 } = {}) => {
    const params = new URLSearchParams();
    if (limit !== null) {
  ...
  // Admin endpoints
  admin: {
    listUsers: () => authenticatedFetch('/api/auth/admin/users'),
    createUser: (username, password) => authenticatedFetch('/api/auth/admin/users', {
      method: 'POST',
      body: JSON.stringify({ username, password }),
    }),
    setUserActive: (userId, isActive) => authenticatedFetch(`/api/auth/admin/users/${userId}`, {
      method: 'PUT',
