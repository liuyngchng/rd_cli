...
export const parseJsonMcpPayload = (
  ...
  return {
    ...
    envVars: (options?.includeProviderSpecificFields ?? false)
      ? readStringArray(parsed.envVars ?? parsed.env_vars) ?? []
      : undefined,
    bearerTokenEnvVar: (options?.includeProviderSpecificFields ?? false)
      ? readString(parsed.bearerTokenEnvVar ?? parsed.bearer_token_env_var)
      : undefined,
    envHttpHeaders: (options?.includeProviderSpecificFields ?? false)
